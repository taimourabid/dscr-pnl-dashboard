...
// Money In
app.post('/api/money-in', (req, res) => {
  const data = readData();
  const txn  = { id: uid(), ...req.body, amount: parseFloat(req.body.amount) };
  data.moneyIn.push(txn);
  writeData(data);
  res.json(txn);
});
app.delete('/api/money-in/:id', (req, res) => {
  const data = readData();
  data.moneyIn = data.moneyIn.filter(t => t.id !== req.params.id);
  writeData(data);
  res.json({ ok: true });
});
...
app.listen(PORT, () => console.log(`DSCR P&L running on port ${PORT}`));
